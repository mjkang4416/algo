...
dp 를 써야하나 생각했다가 예외 케이스가 너무 많을 것 같고,,, 시계 , 반시계 방향으로 돌리는 아이디어가 전혀 생각이 나지 않아서 빠르게 답을 봤다. 

1. 사실상 반시계 방향으로 돌린다 라기 보단, 시작점을 바꾸면서 한바퀴 더 돌린다고 가정한다. 시계문제 아이디어 생각
2. 조합…. 외우자!!!!! 조합을 생각은 했었는데 음. 외우자. 

코드 작성 보다 이미 작성된 코드를 적으면서 이해 해 본거라 다시 생각 해 볼 필요가 있는 문제다. 이것도 항상,,, 인덱스 조심. 배열은 인덱스 틀어지면 너무 꼬이기에.. i 랑 j 반복문 돌릴때 잘 구분하자.
