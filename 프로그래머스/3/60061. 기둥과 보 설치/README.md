...
카카오 치고 무난했던 문제 , 기존에는 전체 체크 말고 안되는 경우를 찾아서 선택적으로 넣으려고 했는데 , 넣는 경우 말고 삭제하는 경우 경우의 수가 너무 많을 것 같아 찾아봤더니 전체검사 로직…. 전체검사도 생각 해 보긴 했는데 너무 비효율 적일 것 같아서 제외시켰건만 … 구현에서는 정말 시간복잡도는 신경 안 써도 되나보다 . 말고는 생각했던 로직이랑 같다. 

- boolean isValid = false; 를 생각 못했다. 즉 if 에서 하나의 조건만 만족하면 다음 루프로 넘어가게 했는데 , 그러면 기둥일 경우만 검사하고 continue 로 넘어가게 된다. 마지막 루프를 안돌릴 경우를 항상 생각하자.
- 분기문에서 if, else if 가 와야할 경우를 잘 생각 못하는 것 같다. if 는 모든 조건을 전부 만족해야 하는 경우 !
- continue 쓸때 다음 루프로 넘어가면 검사 안될 수 있는 부분 생각하자.
