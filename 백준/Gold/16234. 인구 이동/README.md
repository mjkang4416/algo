...
틀을 잘 잡고 들어갈것. 답을 보고 나니 거의 다 왔었는데 .. ~~dfs 는 그냥 같은데 while 쪽을 못잡았다.~~ 

1. 덩어리로 dfs 할때는 돌면서 방문 안한 노드 기준으로 dfs 계속 시키기 (여기선 1,0 으로 벽 같이 구분하지 않으니까 이렇게 방문 했는지 , 안했는지 기준
2. 구현에서는 while 안에 for 문 있어도 타임아웃 안난다. 걱정 하지말고 하자. 
3. 상태 flag 로 체크하기
