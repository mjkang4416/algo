...
별 문제도 아닌걸 오래 풀었다.. 정말 백준은… 백준 쓸 때 마다 오래 걸리는 것 같다. 

사유

1. import 문 땜에 자꾸 .. 런타임 에러가 … 
2. ; 세미콜론 빼먹지 말자 … 
3. 아직 main() 함수 문법 다 쓰는게 익숙하지 않다.. 결국 문법 오류가 제일 많이나서 런타임 에러가 많이 난다. 
4. 문법 외에도 파라미터를 안주거나 하는 사소한 오류가 많이 난다. 프레임워크 안 쓰니까 스스로 얼마나 꼼꼼하지 못 한 지가 보인다. 알고리즘만 생각하다가 문법 상 오류를 내는 경우가 많다. 
5. 알고리즘 적으로 고민을 했던건 3번 ‘연속’ 이어야 한다는것, 즉 띄엄띄엄 3번을 만족하면 안되고 연달아 세면 이어야 한다. 그래서 다음 노드보다 크고 작고 를 if,else 로 나눠서 큰 경우 작은 count 를 초기화 시키는 방식으로 띄엄 띄엄 3번을 방지했다. (다만 else 가 없음,, 동일할 경우에도 초기화 시켜줘야 할 듯 한데 왜 통과했는지 모르겠다 .. ) 
6. 디버깅시 발견한 부분 
myStockNum += remain/StockPrice[i];
                remain %= StockPrice[i];
를 반대로 썼었다. remain%= 를 하면 remain 값이 변경됨으로 기존 remain 값을 사용해서 / 하는 myStockNum 을 먼저 써야 한다. 순서 조심하자.
7. 번외로 , 시점파악 이번에는 좀 잘 된 것 같다. 언제 ++ 을 시킬지가 알고리즘의 결과에 크게 기여하는데, 이 타이밍 (먼저 ++ 일지 마지막에 ++ 일지) 를 잘 보는게 중요할 것 같다.
8. 문제 잘보기.. 이번에도 조건 하나를 못봐서 … 문제 속독하지 말자 !!!!! 
