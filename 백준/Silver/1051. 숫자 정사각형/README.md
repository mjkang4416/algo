...
이런 문제 푸는데 30분이나 걸렸다니.. 전형적인 부르트포스 문제 였고 그렇게 생각하고 풀었다. 

빨리 안됐던건, arr 받아올때 split() 이 없는데 자꾸 손이 자동으로 split() 를 친다 .. 그거 때문에 map 객체로 받아와져서 

디버깅 하느라 좀 느렸다. 나머지는 큰 문제 없었다. 그리고 자꾸 max 빠뜨리는데 최댓값 구할때 항상 신경쓰기
