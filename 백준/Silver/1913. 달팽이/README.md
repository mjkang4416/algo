# [Silver III] 달팽이 - 1913 

[문제 링크](https://www.acmicpc.net/problem/1913) 

### 성능 요약

메모리: 70928 KB, 시간: 1396 ms

### 분류

구현

### 제출 일자

2025년 12월 16일 15:19:35

### 문제 설명

<p>홀수인 자연수 N이 주어지면, 다음과 같이 1부터 N<sup>2</sup>까지의 자연수를 달팽이 모양으로 N×N의 표에 채울 수 있다.</p>

<table class="table table-bordered td-center" style="width:9%">
	<tbody>
		<tr>
			<td style="width:3%">9</td>
			<td style="width:3%">2</td>
			<td style="width:3%">3</td>
		</tr>
		<tr>
			<td>8</td>
			<td>1</td>
			<td>4</td>
		</tr>
		<tr>
			<td>7</td>
			<td>6</td>
			<td>5</td>
		</tr>
	</tbody>
</table>

<table class="table table-bordered td-center" style="width:15%">
	<tbody>
		<tr>
			<td style="width:3%">25</td>
			<td style="width:3%">10</td>
			<td style="width:3%">11</td>
			<td style="width:3%">12</td>
			<td style="width:3%">13</td>
		</tr>
		<tr>
			<td>24</td>
			<td>9</td>
			<td>2</td>
			<td>3</td>
			<td>14</td>
		</tr>
		<tr>
			<td>23</td>
			<td>8</td>
			<td>1</td>
			<td>4</td>
			<td>15</td>
		</tr>
		<tr>
			<td>22</td>
			<td>7</td>
			<td>6</td>
			<td>5</td>
			<td>16</td>
		</tr>
		<tr>
			<td>21</td>
			<td>20</td>
			<td>19</td>
			<td>18</td>
			<td>17</td>
		</tr>
	</tbody>
</table>

<p>N이 주어졌을 때, 이러한 표를 출력하는 프로그램을 작성하시오. 또한 N<sup>2</sup> 이하의 자연수가 하나 주어졌을 때, 그 좌표도 함께 출력하시오. 예를 들어 N=5인 경우 6의 좌표는 (4,3)이다.</p>

### 입력 

 <p>첫째 줄에 홀수인 자연수 N(3 ≤ N ≤ 999)이 주어진다. 둘째 줄에는 위치를 찾고자 하는 N<sup>2</sup> 이하의 자연수가 하나 주어진다.</p>

### 출력 

 <p>N개의 줄에 걸쳐 표를 출력한다. 각 줄에 N개의 자연수를 한 칸씩 띄어서 출력하면 되며, 자릿수를 맞출 필요가 없다. N+1번째 줄에는 입력받은 자연수의 좌표를 나타내는 두 정수를 한 칸 띄어서 출력한다.</p>

 ## 소감

논리 부족으로 결국 답을 본 문제 .. 쉬운 문제였느데 왜 기본적인 dfs bfs 아니면 생각을 못하냐 !!! 결국 하 오 위 왼 방향으로

벽을 만날때까지 계속 가면서 반복하는 구조다. 처음에 1부터 시작해서 dfs 로 풀려 했지만 벽을 만날수가 없기에 .. 이건 불가

해서 범위를 점점 늘리면서 벽 자체를 만들어 줘야 하나 .. ? 라고 생각했는데 너무 말도안되는 발상이어서 집어치우고 답을 봤다.

1. 처음부터 시작해서 숫자를 줄이는 것도 생각, 안되면 뒤집에서 생각 해 보는 것도 필요하다. 
2. dfs  가 아닐수도 있다. 한 방향으로 쭉 가기만 하면 되니, while 처리 하며 쭉 가도 된다.  ( 근데 사실상 dfs 코드를 살짝 변형
    
    시킨거긴 하다 .. ) 
    
3. while 문 돌릴때 체크 포인트 검사를 잘 하자. break 전에 체크해야 할 게 있다면 해당 if 문을 위로 올려야 한다. 체크 안되고
    
    멈추는게 있는지 항상 체크 할 것 .

