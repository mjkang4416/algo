...
논리를 못잡은 문제, 어떻게 해야할지 아이디어가 전혀 떠오르지 않았다. 결론적으로 순열을 이용해 모든 경우의 수를 생성 한 다음

일치하는 것만 count +1 하는 방식 이었는데 이 전체 경우의 수를 만드는 아이디어를 떠올리지 못한게 문제의 원인이 아니었나 싶

다. 

결국 

1. 모든 경우의 수에 대해 질문 List 를 순회하며 strike, ball 개수가 맞는지 확인
2. 해당 경우의 수가 모든 질문 List 에 대해 strike, ball 개수가 일치하면 후보!

⇒ 구현시 뭐가 안떠오르면 모든 경우의 수에 대해 완탐 한다고 생각하자.
