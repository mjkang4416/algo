...
오래걸릴 문제도 아니었는데 도저히 안 떠올라서 답을본 문제 .. 내거랑 크게 다르지는 않았는데, 내가 좀 더 복잡하게 생각한 듯 하다. 

결국 연산자를 순열로 배열해야 함으로 백트레킹을 떠올렸는데 중복은 허용하지 않는다 를 어떻게 처리해야 할까를 고민하다가  - - 하는 방식으로 0 이면 선택하지 못하게 하도록 처리했다. (이부분은 동일)

나는 선택한 연산자를 따로 selected List 를 만들어서 매번 새로 만들로 if idx 가 연산자 숫자 만큼 차면 만든 list 를 사용해서 연산하려 했다. 근데 이렇게 하면 selected List 를 만들때 

1. 우리는 연산자 숫자가 적힌 list 를 4 번 돌아야 하는데 selectedList 에 선택된 애를 넣을때 i 가 없다 .. 

그래서 찾아보니 다른 사람들은 if 에서 따로 연산을 안하고 미리 연산을 해서 넣어버리는것 ! 그럼 selected list 가 필요 없기 때문에 저렇게 꼬이는게 안생긴다. 

⇒ 너무 로직 분리하지 말고 한번에 할 수 있는건 파라미터로 넘기자 !
