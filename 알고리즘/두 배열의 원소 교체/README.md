...
하나는 오름차순, 하나는 내림차순으로 바꿔주고 체인지 횟수만큼 swap 해 주면 된다. 

다만 자바의 오름, 내림차순 정렬이 조금 생소했는데 

Arrays.sort(B, Comparator.reverseOrder()); 

여기서 기본 오름차순 정렬이 아닌 내림차순 정렬을 위해 Comparator 비교기준을 지정해서 정렬 해 준다. 여기서

Comparator 인터페이스는 객체 끼리 비교하게 구현 돼 있음으로 Integer 레퍼런스 타입을 사용 해 줘야 한다. 

레퍼런스 타입이지만,, 언박싱 일어나서 int 에 넣는데는 문제 없다고 함!
