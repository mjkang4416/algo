...
다익스트라 구현 자체가 어려워서 헤맷던 문제 큐를 쓰는게 더 효율적 이라고는 하지만.. 그냥 list 쓰는게 직관적인 것 같기는 하다. 다익스트라는 하나의 노드에서 모든 노드로 갈 수 있는 최단경로를 구하는건데 모든 노드에서 갈 수 있다고 착각해서 개념적으로 이해하는데 시간이 걸렸다..  ~~이제 사피도 얼마 안남았으니까 좀 열심히 하자 .. 소마는 어케 칠라고 그러니 ..~~  기존에 처음푸는 알고리즘을 혼자 풀어보겠다고 끙끙대는데 너무 시간을 투가 한 것 같아서 이번엔 희망이 안보이면 빨리 포기하고 이해하고 외우는데 치중했다. ~~실재로 설탕도 외워서 풀었으니까 ….~~ 감 만 잃지 않으면 방법을 외우는 것도 하나의 방법 일지도 ?
