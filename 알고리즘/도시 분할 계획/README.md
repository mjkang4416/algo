...
저번에 풀 때 union 을 잘못 이해하고 풀어서 시간이 좀 걸렸다. union 은 find 로 찾은 나의 부모노드가 나른 노드

의 부모노의 보다 작은경우 해당 부모노드들 끼리 우선순위를 달리 해주는 건데 현재 노드를 다른 노드의 부모노드 

에게 붙여준다고 생각했다. ( 부모노드 끼리 붙는것 !!! ) 외에는 신장트리를 두개의 부분 그래프로 나눌 때 가장 큰 

간선을 하나 빼 주면 최소 값으로 나뉜다 라는 아이디어를 생각해 내는게 어려웠다. ~~생각해보면 당연한것 ..~~
