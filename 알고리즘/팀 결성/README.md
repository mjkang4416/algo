...
코드는 긴데 크게 사고력을 요하는 문제는 아니었다. union 이랑 find 만 잘 만들 수 있으면 편하게 풀리는 문제

다만 java 에는 map 이 없어서 .. 좀 자증난다… Node 객체 만들어서 arr 에 넣는거 말고 더 좋은 방법 없을까 ? 

이차원 배열 만들어서 하나에 3개 받아 버리는 것도 방법일듯 그게 더 구현은 편했을 것 같은데 객체 만드는 거 좀 

익숙해 져보자 하는 취지로 하나 만들어 봤다. 이번엔 개념파트 설명을 좀 열심히 봤는데 역시 개념을 재대로 알

아야 알고리즘도 정확하게 풀수 있는 듯 함.
